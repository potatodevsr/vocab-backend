...
export function AdminUserDocs(config: DocsConfig = {}) {
  return (req: Request, res: Response) => {
    ...
    if (ui === "playground") {
      ...
      return res.type("html").send(renderPlayground("AdminUser", config));
    }
    ...
    const pageTitle = config.docsTitle || `AdminUser API`;

    if (ui === "scalar") {
      return res
        .type("html")
        .send(renderScalar("AdminUser", spec, pageTitle, config.scalarCdnUrl));
    }

    const html = renderDocs("AdminUser", config, MODEL_CONTEXT);
